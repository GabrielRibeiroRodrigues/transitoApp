...
const FormComponent = ({database, tabelas = [], fields, onSubmit , initialData = {}},edicao = {}, ocultar) => {
  const [formData, setFormData] = useState(initialData);
  const [dateError, setDateError] = useState(null);
  const [inlineFields, setInlineFields] = useState({});
  const [campos, setCampos] = useState(null);
  const [cc, setcc] = useState();
  const reg = ["Chevrolet", "Volvo"]
  const [data, setData] = useState([]);
  // Como Ler o tipo que o formulario vai suportar ????
  ...
  useEffect(() => {
    ...
    console.log('edit resposta form comp',edicao);
      setFormData(initialData);
    
   // Atualização com dados iniciais
  }, [initialData]);// Executa apenas uma vez na montagem do componente
  useEffect(() => {
    
    const fetchData = async () => {
      try {
        console.log(`Fetching data from database: ${database}, table: ${tabelas}`);
        const result = await databasee.getAllRecords(database, tabelas);
    ...
    const fetchCampos = async () => {
      try {
        const schema = await databasee.getTableSchema(database, tabelas);
        console.log('Schema da tabela:', schema);
        const mac = gerarFields(schema,ocultar); // Gera os 
        console.log('DENTRO DA FUNÇÃO', ocultar);
        console.log('mac antes:', mac);
        setCampos(mac);
        console.log('mac depois:', mac);
        // Atualiza o estado com os fields
      } catch (error) {
...
// console.log('Teste', teste);
  ...
  return (
    <ScrollView style={styles.formContainer}>
      {fields.map(renderField)}
      <Pressable style={styles.submitButton} onPress={() => onSubmit(formData)}>
